...
if (typeof lastPastedValueMap === "undefined") {
    var lastPastedValueMap = new Map();
}

// ?? Track whether the popup should be displayed
if (typeof popupEnabled === "undefined") {
    var popupEnabled = true;
}

// ?? Ensure the clipboard popup only appears when enabled
chrome.storage.local.get({ popupEnabled: true }, (data) => {
    window.popupEnabled = data.popupEnabled;
});

// ?? Listen for storage changes & apply instantly
chrome.storage.onChanged.addListener((changes) => {
    if (changes.popupEnabled) {
        window.popupEnabled = changes.popupEnabled.newValue;
    }
});
// Show popup when clicking inside an input field
// ?? Show popup when clicking inside an input field (only if enabled)
document.addEventListener("focusin", (event) => {
    if (!window.popupEnabled) return;
...
// ?? Ensure clipboard popup closes on right-click inside an input field
document.addEventListener("contextmenu", (event) => {
    let popup = document.getElementById("clipboard-popup");
    if (popup) {
        popup.remove();  // ✅ This ensures the popup disappears instantly
    }
});
...
function showClipboardPopup(targetField, clipboardData) {
    ...
    chrome.storage.local.get(["popupSize"], (data) => {
        let popupWidth = data.popupSize?.width || 400; // Default width
        let popupHeight = data.popupSize?.height || 200; // Default height
        ...
        popup.style.width = `${popupWidth}px`;  // Set stored width
        popup.style.height = `${popupHeight}px`;  // Set stored height
        popup.style.minWidth = "250px"; // Prevent shrinking too much
        popup.style.minHeight = "150px"; // Prevent shrinking too much
        ...
        // **Header container to hold buttons**
        let header = document.createElement("div");
        header.style.display = "flex";
        header.style.justifyContent = "flex-end"; 
        // header.style.justifyContent = "space-between";
        header.style.width = "100%";
        header.style.paddingBottom = "8px"; // Space below buttons
        ...
        clearButton.style.transition = "background 0.2s";
        clearButton.style.marginRight = "5px"; // Adjust margin
        ...
        clearButton.addEventListener("click", () => {
            chrome.storage.local.set({ clipboard: [] }, () => {
                popup.remove(); // Remove popup after clearing
                console.log("All clipboard items cleared.");
            });
        });
        ...
        dragHandle.innerText = "☰"; // Three-line menu icon
        ...
        dragHandle.addEventListener("mousedown", (event) => {
            isDragging = true;
            event.stopPropagation(); // Prevent closing when clicking the handle
        ...
        document.addEventListener("mouseup", () => {
            isDragging = false;
            attachOutsideClickListener(popup, targetField); // Ensure clicking outside works again
        });
        ...
        closeButton.style.marginRight = "8px"; // Adjust margin
        closeButton.style.marginLeft = "8px"; // Adjust margin
        ...
        clipboardData.forEach((text, index) => {
         ...
         resizeHandle.addEventListener("mousedown", (event) => {
             ...
             document.addEventListener("mouseup", () => {
                 ...
                 chrome.storage.local.set({ popupSize: { width: popup.offsetWidth, height: popup.offsetHeight } });
             }, { once: true });
         });

         function resizePopup(event) {
             let newWidth = event.clientX - popup.getBoundingClientRect().left;
             let newHeight = event.clientY - popup.getBoundingClientRect().top;
             popup.style.width = `${Math.max(250, newWidth)}px`;
             popup.style.height = `${Math.max(150, newHeight)}px`;
         }
       ...
       // Ensure Popup Closes When Clicking Outside
        // Function to close the popup when clicking outside (AFTER dragging)
        function attachOutsideClickListener(popup, targetField) {
            setTimeout(() => {
        ...
        attachOutsideClickListener(popup, targetField);
